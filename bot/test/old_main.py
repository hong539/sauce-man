...
class MyClient(discord.Client):
    def __init__(self, path: str) -> None:
        ...
        self.guild = discord.Object(
            id=int(self.config_data["bot"]["guild_id"])
...
def main():
    ...
    async def add(
        interaction: discord.Interaction, first_value: int, second_value: int
    ):
    ...
    @client.tree.command()
    @app_commands.describe(
        member="The member you want to search", channel="The channel you want to search"
    )
    async def search_oldest_message(
        ...
        await interaction.response.send_message(
            f"member: {member} in channel: {channel}: send an oldest message: {content} at jump_url: {jump_url}"
        )

    @client.tree.command()
    @app_commands.describe(
        member="The member you want to dump",
    ...
    async def dump_oldest_message(
        ...
        before_date = (
            datetime.datetime.fromisoformat(before_date) if before_date else None
        )
        ...
        async for message in channel.history(
            limit=None, before=before_date, after=after_date, oldest_first=True
        ):
            counter += 1
